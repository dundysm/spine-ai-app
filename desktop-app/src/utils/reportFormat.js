/**
 * Convert Markdown to sanitized HTML and plain text for medical reports.
 */
...
export function htmlToFormattedText(html) {
  ...
  const walk = (node, lines = []) => {
    ...
    const tag = node.tagName?.toLowerCase()
    if (tag === 'h1') { lines.push(''); lines.push((node.textContent || '').trim().toUpperCase()); lines.push(''); return lines }
    if (tag === 'h2' || tag === 'h3') { lines.push(''); lines.push((node.textContent || '').trim()); lines.push(''); return lines }
    if (tag === 'p' || tag === 'div') { for (const child of node.childNodes) walk(child, lines); lines.push(''); return lines }
    if (tag === 'li') { lines.push('  • ' + (node.textContent || '').trim()); return lines }
    if (tag === 'br') { lines.push(''); return lines }
