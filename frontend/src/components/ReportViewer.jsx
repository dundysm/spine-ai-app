...
// Configure marked for clean medical report output
marked.setOptions({
  gfm: false,
  breaks: true,
  headerIds: false,
...
function ReportViewer({
  ...
  // When report from API changes: convert markdown to HTML and reset edit state
  useEffect(() => {
  ...
  const copyToClipboard = useCallback(() => {
    ...
    if (navigator.clipboard?.writeText) {
      ...
      alert('Report copied to clipboard. Paste into your report with Ctrl+V.')
    } else {
    ...
    >
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {report && (
            <button
              ...
              title="Copy report to clipboard (Ctrl+V to paste)"
            >
          ...
          {report && viewMode && (
            <button
              ...
              className="px-3 py-2 rounded-xl border border-[var(--color-border)] text-[var(--color-text)] hover:bg-[var(--color-bg-subtle)] font-medium text-sm transition-all duration-200"
              title="Edit report"
            >
              Edit Report
            </button>
          ...
          {report && !viewMode && (
            <button
              type="button"
              onClick={saveEdit}
              className="px-3 py-2 rounded-xl font-medium text-sm text-white bg-[var(--color-primary)] hover:bg-[var(--color-primary-dark)] transition-all duration-200"
              title="Save and return to view"
            >
              Save Changes
            </button>
          )}
          <button
            ...
            className="inline-flex items-center gap-2 px-4 py-2 rounded-xl font-medium text-white bg-gradient-to-r from-[var(--color-primary)] to-[var(--color-primary-dark)] hover:shadow-panel-soft disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-250 ease-out-expo"
            title={hasStudy && !analyzing ? 'Run AI analysis on the loaded study' : undefined}
          >
            {analyzing ? (
              <>
                <span className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-medical-spin" aria-hidden />
                Analyzing…
              </>
            ) : (
              'Analyze with AI'
          ...
          {report && (
            <div className="relative">
              <button
                ...
                className="px-4 py-2 rounded-xl border border-[var(--color-border)] text-[var(--color-text)] hover:bg-[var(--color-bg-subtle)] font-medium transition-all duration-200 text-sm"
                title="Export report"
              >
                Export ▾
              </button>
          ...
          <div className="flex flex-wrap gap-2">
            {DISC_LEVELS.map((level) => {
              ...
              return (
                <span
                  key={level}
                  className={`inline-flex items-center gap-1 px-2 py-0.5 rounded border text-xs font-medium ${bg} ${isLow || isMedium ? 'text-amber-900' : 'text-green-900'}`}
                  title={isLow || isMedium ? 'Verify this finding carefully' : 'High confidence'}
                >
      ...
      {hasLowConfidence && report && (
        <div className="mx-4 mt-2 py-2 px-3 rounded-panel bg-amber-50 border border-amber-200 text-amber-900 text-sm shrink-0 animate-fade-in">
          <span className="font-medium">Please verify:</span> Findings marked Low or Medium confidence; review these levels on the images before signing.
        </div>
      )}

      {/* Content: view (formatted HTML) or edit (Quill) or empty/skeleton */}
      <div className="flex-1 min-h-0 flex flex-col overflow-hidden">
        {analyzing && !report ? (
          <div className="flex-1 p-6 space-y-4 animate-fade-in" aria-live="polite" aria-busy="true">
            ...
            <div className="h-4 rounded-lg w-1/2 mt-6 skeleton-shimmer" style={{ animationDelay: '0.25s' }} />
            <div className="h-3 rounded w-full skeleton-shimmer" style={{ animationDelay: '0.3s' }} />
            <div className="h-3 rounded w-4/5 skeleton-shimmer" style={{ animationDelay: '0.35s' }} />
          </div>
        ) : !report && !analyzing ? (
          <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
            <svg
              className="w-20 h-20 text-[var(--color-text-muted)] opacity-60 mb-4 animate-pulse-soft"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
              strokeWidth="1"
            ...
            <p className="text-gray-500 font-medium">Load a study and click “Analyze with AI” to generate a report.</p>
            <p className="text-sm text-[var(--color-text-muted)] mt-1 opacity-80">Reports appear here for review and export.</p>
          </div>
        ) : viewMode ? (
          <div
            className="flex-1 min-h-0 overflow-y-auto overflow-x-hidden p-5 medical-report bg-[var(--color-bg-elevated)]"
            role="article"
          ...
          <div className="flex-1 min-h-0 flex flex-col overflow-hidden report-editor-wrapper">
            <ReactQuill
              ...
              modules={modules}
              className="flex-1 flex flex-col quill-report report-editor min-h-0"
      ...
      <div className="shrink-0 px-4 py-2 border-t border-[var(--color-border)] text-[var(--color-text-muted)] text-xs bg-[var(--color-bg-subtle)]">
        {analyzing ? (
          <span className="flex items-center gap-2">
            <span className="inline-block w-4 h-4 border-2 border-[var(--color-primary)] border-t-transparent rounded-full animate-spin" aria-hidden />
            AI analysis in progress…
          </span>
        ) : report ? (
          <span>
            {viewMode ? 'View mode. Click “Edit Report” to modify.' : 'Edit mode. Click “Save Changes” when done.'}
            {Object.keys(levelConfidence).length > 0 && ' Confidence by level above.'}
          </span>
        ) : (
          <span>Upload a study and run analysis to generate a report.</span>
        )}
      </div>
    </div>
  )
